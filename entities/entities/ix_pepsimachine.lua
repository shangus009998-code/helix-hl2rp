...
if (SERVER) then
	...
	util.AddNetworkString("ixPepsiMachineReset")

	net.Receive("ixPepsiMachineReset", function(len, client)
		if (client:IsAdmin()) then
			local entity = net.ReadEntity()

			if (IsValid(entity) and entity:GetClass() == "ix_pepsimachine") then
				entity:ResetStock()
	...
	properties.Add("ixPepsiMachineReset", {
		MenuLabel = L("vendingResetStock"),
		Order = 999,
		MenuIcon = "icon16/arrow_refresh.png",
		Filter = function(self, entity, client)
			if (!IsValid(entity) or entity:GetClass() != "ix_pepsimachine") then return false end
			if (!client:IsAdmin()) then return false end

			return true
		end,
		Action = function(self, entity)
			net.Start("ixPepsiMachineReset")
