...
function ENT:OnPopulateEntityInfo(tooltip)
	...
	name:SetImportant()
	name:SetText(customName or (isClawScanner and L("scannerNameClaw") or L("scannerName")))
	name:SetBackgroundColor(team.GetColor(FACTION_MPF))
	name:SizeToContents()

	local desc = tooltip:AddRow("desc")
	desc:SetText(L("scannerDesc"))
	desc:SizeToContents()
	
	local pilot = self:GetPilot()
	if (IsValid(pilot) and pilot:GetCharacter()) then
		local client = LocalPlayer()
		local bHasKey = false
		local inv = client:GetCharacter() and client:GetCharacter():GetInventory()

		if (inv) then
			bHasKey = inv:HasItem("comkey")
		end

		if (client:IsAdmin() or (client:IsCombine() and Schema:CanPlayerSeeCombineOverlay(client)) or bHasKey) then
			local pilotRow = tooltip:AddRow("pilot")
			pilotRow:SetText(L("scannerPilot", pilot:GetCharacter():GetName()))
			pilotRow:SetBackgroundColor(Color(60, 120, 60))
			pilotRow:SizeToContents()
		end
	end
